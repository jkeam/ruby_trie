...
describe RubyTrie::Trie do
  ...
  describe "when asked to add" do
    it "must add it" do
      @trie.add 'kitten'
      root = @trie.get_node 'k'

      i_actual = root.get_node 'i'
      i_actual.wont_be_nil

      t_actual = i_actual.get_node 't'
      t_actual.wont_be_nil

      t_actual = t_actual.get_node 't'
      t_actual.wont_be_nil

      e_actual = t_actual.get_node 'e'
      e_actual.wont_be_nil

      n_actual = e_actual.get_node 'n'
      n_actual.wont_be_nil

      m_actual = e_actual.get_node 'm'
    ...
    it "must print out correctly" do
      ...
      k_actual = root.get_node :k
      k_actual.to_s.must_equal "{Node: value: k, parent_value: }"

      i_actual = k_actual.get_node :i
